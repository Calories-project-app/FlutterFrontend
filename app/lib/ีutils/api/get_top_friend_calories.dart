...
class FoodStreakAPI {
  static Future<List<FoodStreak>> fetchFoodStreakFriends(String userId) async {
    final response = await http.get(Uri.parse(
        'https://foodcal-api-latest.onrender.com/users/food-streak-friends?userId=$userId'));

    if (response.statusCode == 200) {
      List<dynamic> data = jsonDecode(response.body);
      List<FoodStreak> foodStreakFriends =
          data.map((json) => FoodStreak.fromJson(json)).toList();
